...
def closest_centroid(
    centroid_goal: Tuple[float, float], centroids_list: List[Tuple[float, float]]
) -> Tuple[float, float]:
    """
...
for (
    ...
    confidences,
    class_ids,
), (frame_id, centroids, motion_frame) in zip(gen_yolo, gen_motion):
    ...
    cv2.putText(
        frame_resized, f"YOLO", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2
    )
    cv2.putText(
        frame_resized, f"OpenCV", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2
    )
    cv2.putText(
        frame_resized,
        f"Butterworth",
