...
def drone_space_to_camera_space(
    camera_obj: bpy.types.Object, FOV: float, borne_dist: Tuple[float, float]
) -> mathutils.Vector:
...
def move_scene(
    drone_obj: bpy.types.Object,
    borne_dist: Tuple[float, float],
    theta_bornes: Tuple[float, float],
) -> list:
    ...
    move_infos = drone_space_to_camera_space(camera_obj, math.radians(60), borne_dist)
...
def give_output(
    ...
    for i in range(n):
        ...
        # On export l'image
        render_and_save(
            f"ts341_project/model_training/sim2real_approach/dataset/images/image_{i:03d}.png"
        )

        # On export le label correspondant
        with open(
            f"ts341_project/model_training/sim2real_approach/dataset/labels/image_{i:03d}.txt",
            "w",
        ) as f:
...
def main() -> None:
    ...
    bpy.ops.wm.open_mainfile(
        filepath="ts341_project/model_training/sim2real_approach/base.blend"
    )
